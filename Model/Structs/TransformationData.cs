...
namespace DBZGoatLib2.Model {
    ...
    public readonly struct AuraData {
        ...
        public Texture2D GetTexture() => ModContent.Request<Texture2D>(string.IsNullOrEmpty(AuraPath) ? "DBZGoatLib2/Assets/BaseAura" : AuraPath, AssetRequestMode.AsyncLoad).Value;
        ...
        public Tuple<float, Vector2> GetAuraRotationAndPosition(GPlayer modPlayer, float auraScale) {
            ...
            Vector2 vector2;
            /*
            dynamic DBZModPlayer = DBZGoatLib2.DBZMOD.Value.mod.Code.DefinedTypes.First(x => x.Name.Equals("MyPlayer")).GetMethod("ModPlayer").Invoke(null,
                [modPlayer.Player]);
