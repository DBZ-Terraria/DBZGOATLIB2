...
namespace DBZGoatLib2.Model {
    ...
    public abstract class Transformation : ModBuff, IDBZLoadable {
        ...
        /// <summary>
        /// Either technique or transformation, replacement for "stackable"
        /// </summary>
        ...
        public TransformationInfo Info => new TransformationInfo(
            Type, Name, FormType(), FormName(), TextColor(), CanTransform, OnTransform, PostTransform, AnimationData, KiBarGradient());
        ...
        public override void Update(Player player, ref int buffIndex) {
            ...
            FieldInfo KiDamage = DBZGoatLib2.DBZMOD.Value.mod.Code.DefinedTypes.First(x => x.Name.Equals("MyPlayer")).GetField("KiDamage");
            FieldInfo KiDrainRate = DBZGoatLib2.DBZMOD.Value.mod.Code.DefinedTypes.First(x => x.Name.Equals("MyPlayer")).GetField("kiDrainMulti");
            
            if (modPlayer.IsKiDepleted() || (TransformationHandler.IsAnythingBut(player, Type) && FormType() == "transformation")) {
                TransformationHandler.ClearTransformations(player);
                return;
            }
            */

            if (TransformationHandler.IsKaioken(player) && FormType() == "transformation")
            {
            ...
            if(speedMulti != 0)
            {
                player.moveSpeed *= 1f + (speedMulti - 1f);
                player.maxRunSpeed *= 1f + (speedMulti - 1f);
                player.runAcceleration *= 1f + (speedMulti - 1f);
                if (player.jumpSpeedBoost < 1f)
                {
                    player.jumpSpeedBoost = 1f;
                }

                player.jumpSpeedBoost *= 1f + (speedMulti - 1f);
            }
