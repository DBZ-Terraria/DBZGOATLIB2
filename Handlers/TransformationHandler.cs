...
namespace DBZGoatLib2.Handlers
{
    public sealed class TransformationHandler
    {
            ...
            get
            {
                ...
                if (dbtTransformInfo == null)
                {
                    ...
                    for (int i = 0; i < DBTForms.Count; i++)
                    {
                        ...
                        ModBuff buff = DBZGoatLib2.DBZMOD.Value.mod.Find<ModBuff>(buffName);
        ...
        private static bool DBTFormCheck(string BuffName, Player player)
        {
            ...
            dynamic modPlayer = DBZGoatLib2.DBZMOD.Value.mod.Code.DefinedTypes.First(x => x.Name.Equals("MyPlayer"))
        ...
        /// <summary>
        /// The Transform keybind.
        /// </summary>
        public static ModKeybind TransformKey = null!;

        /// <summary>
        /// The Technique keybind.
        /// </summary>
        public static ModKeybind TechniqueKey = null!;

        /// <summary>
        /// The Power down keybind.
        /// </summary>
        public static ModKeybind PowerDownKey = null!;

        /// <summary>
        /// The Energy Charge keybind.
        /// </summary>
        ...
        /// <summary>
        /// Unregisters a transformation chain.
        /// </summary>
        /// <param name="chain"></param>
        ...
        private static void ExternalTransform(Player player, TransformationInfo form)
        {
            ...
            TypeInfo tHandler =
                DBZGoatLib2.DBZMOD.Value.mod.Code.DefinedTypes.First(x => x.Name.Equals("TransformationHelper"));
            ...
            MethodInfo canTransform = tHandler.GetMethod("CanTransform", [ 
                typeof(Player),
                DBZGoatLib2.DBZMOD.Value.mod.Code.DefinedTypes.First(x => x.Name.Equals("BuffInfo")).AsType()
            ]);
            MethodInfo doTransform = tHandler.GetMethod("DoTransform");

            if (!(bool)canTransform.Invoke(null, [player, buffInfo]))
                return;
            doTransform.Invoke(null, [player, buffInfo, DBZGoatLib2.DBZMOD.Value.mod]);
        }
        ...
        public static void ClearTransformations(Player player)
        {
            ...
            SoundHandler.PlaySound("DBZGoatLib2/Assets/Sounds/PowerDown", player, 0.3f);
        ...
        public static void ClearTechniques(Player player)
        {
            TypeInfo helper =
                DBZGoatLib2.DBZMOD.Value.mod.Code.DefinedTypes.First(x => x.Name.Equals("TransformationHelper"));
            ...
            foreach (TransformationInfo transformation in Transformations.Where(x => x.type == "technique"))
            {
                EndTranformation(player, transformation);
            }
        }
        ...
        public static bool IsTransformed(Player player, bool IgnoreStackable = false)
        {
            foreach (TransformationInfo trans in Transformations)
            {
                ...
                {
                    if (IgnoreStackable && trans.type == "technique")
                    {
                        continue;
                    }
        ...
        public static TransformationInfo? GetCurrentTransformation(Player player)
        {
            ...
            if (ModLoader.HasMod("DBZMODPORT")) {
                foreach (TransformationInfo transformation in DBT_Transformation_Info.Where(x => x.type != "technique"))
                {   
                    if (player.HasBuff(transformation.buffID))
                    {
                        return transformation;
        ...
        /// <summary>
        /// Gets the current technique the player has. Returns null if none is found
        /// </summary>
        public static TransformationInfo? GetCurrentTechnique(Player player)
        {
            foreach (TransformationInfo transformation in Transformations.Where(x => x.type == "technique"))
            {
                if (player.HasBuff(transformation.buffID))
        ...
        /// <summary>
        /// Gets ALL current transformations the player has, including techniques ones. Returns an empty array if the user is not transformed.
        /// </summary>
        ...
        public static bool IsAnythingBut(Player player, int buffId, bool includeExternal = false)
        {
            ...
            if (ModLoader.HasMod("DBZMODPORT") && includeExternal)
            {
                ...
                {
                    if (player.HasBuff(ext.buffID) && ext.buffID != buffId && ext.type != "technique")
                        return true;
                }
            }
